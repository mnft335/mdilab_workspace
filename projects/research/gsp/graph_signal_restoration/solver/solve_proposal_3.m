function result = solve_proposal_3(shared_config, specific_config)
    ...
    problem_config.L = @(z) {shared_config.Phi(z{1}), ...
                             sqrt(shared_config.G.weights) .* shared_config.G.Diff * z{1} - z{2}};
    ...
    problem_config.prox_g = @(z, gamma) {prox_box(z{1}, shared_config.lower, shared_config.upper), ...
                                         prox_l2(z{2}, gamma{2}, 1 - specific_config.alpha)};
    ...
    algorithm_config.Gamma_y = {1, ...
                                1 / (sqrt(max(eig(shared_config.G.Diff.' * diag(shared_config.G.weights) * shared_config.G.Diff)))) + 1};
