...
for i = 1:numel(field_names)
    ...
    nmse_collection = compute_nmse_from_result(optimal_grid_result_collection.(field_names{i}).result);

    % Get the dimension of the random seed for the corrupted graph
    dimension = find_parameter_dimension(optimal_grid_result_collection.(field_names{i}).grid_config_collection, "corrupted_graph.forward_weight_corruption.idx_to_corrupt.random_seed");

    % Average the NMSE over the random seed dimension
    averaged_nmse_collection{i} = mean(nmse_collection, dimension);

end

% For testing
nmse = cell(numel(averaged_nmse_collection{1}), 1);
for i = 1:numel(averaged_nmse_collection{1})

    nmse{i} = cellfun(@(nmse_on_method) nmse_on_method(i), averaged_nmse_collection);
