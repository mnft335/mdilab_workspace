function result = solver_proposal_3(config_proposal_3)
    ...
    prox_conj_l2_ball = prox.conjugate(@(z, gamma) prox.l2_ball(z, gamma, config_proposal_3.observed_signal, config_proposal_3.radius_l2_ball));
    ...
    formulation_config.L = @(z) {config_proposal_3.observation_operator(z{1}), ...
                                 config_proposal_3.laplacian_operator(z{1}) - config_proposal_3.incidence_operator_adjoint(z{2})};
    formulation_config.Lt = @(z) {config_proposal_3.observation_operator_adjoint(z{1}) + config_proposal_3.laplacian_operator_adjoint(z{2}), ...
                                  - config_proposal_3.incidence_operator(z{2})};
    formulation_config.grad_f = @(z) {zeros(size(z{1})), ...
                                      zeros(size(z{2}))};
    formulation_config.prox_g = @(z, gamma) {prox.box(z{1}, gamma{1}, config_proposal_3.signal_lower_bound, config_proposal_3.signal_upper_bound), ...
                                             prox.l1(z{2}, gamma{2}, config_proposal_3.coefficient_l1)};
