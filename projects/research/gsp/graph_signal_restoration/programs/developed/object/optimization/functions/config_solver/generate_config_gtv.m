function config_gtv = generate_config_gtv(graph, true_signal, observation_model)
    ...
    config_gtv.weighted_differential_operator = @(z) sqrt(graph.weights) .* (graph.Diff * z);
    config_gtv.weighted_differential_operator_adjoint = @(z) graph.Diff.' * sqrt(graph.weights) .* z;
    ...
    config_gtv.coefficient_l1 = 1 / 2;

    % Define config_gtv
    config_gtv.step_size_primal_variable = 1 / (1 + sqrt(max(eig(graph.Diff.' * diag(graph.weights) * graph.Diff))));
    config_gtv.step_size_dual_variable_l2_ball = 1;
    config_gtv.step_size_dual_variable_l1 = 1 / sqrt(max(eig(graph.Diff.' * diag(graph.weights) * graph.Diff)));
