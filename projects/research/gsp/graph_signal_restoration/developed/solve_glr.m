function result = solve_glr(shared_config, specific_config)
    ...
    problem_config.prox_h_conj = {prox_conj(@(z, gamma) prox_ball_l2(z, shared_config.b, shared_config.epsilon)), ...
                                  prox_conj(@(z, gamma) prox_l2(z, gamma, 1 / 2))};

    algorithm_config.x_init = {zeros(shared_config.G.N, 1)};
    algorithm_config.y_init = {zeros(shared_config.G.N, 1), zeros(shared_config.G.N, 1)};
    algorithm_config.Gamma_x = {1 / (1 + shared_config.G.lmax)};
    algorithm_config.Gamma_y = {1, 1 / shared_config.G.lmax};
