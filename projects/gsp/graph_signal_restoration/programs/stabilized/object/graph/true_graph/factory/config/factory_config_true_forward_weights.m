function config_true_forward_weights = factory_config_true_forward_weights(param, arg)
    ...
    switch param.type
        ...
        % Sample true forward weights from the uniform distribution
        case "uniform"

            % Create a function handle that generates true forward weights
            random_stream = create_random_stream(param.random_seed, "generate_true_forward_weights:uniform");
            config_true_forward_weights.generate_true_forward_weights = @(initial_forward_weights) sample_uniform(random_stream, [numel(initial_forward_weights), 1], param.lower_bound, param.upper_bound);

            % Create the configuration name
            config_true_forward_weights.configuration_name = {"true_forward_weights=" + param.type, ...
                                                              "random_seed=" + string(param.random_seed), ...
                                                              "lower_bound=" + string(param.lower_bound), ...
                                                              "upper_bound=" + string(param.upper_bound)};
