...
prox_l1_dual = @(arg) arg - gamma_y1 * prox_l1(arg / gamma_y1, 1 / gamma_y1, alpha);
prox_ball_l2_dual = @(arg) arg - gamma_y2 * prox_ball_l2(arg / gamma_y2, b, epsilon);

for i = 1:iter
    u_prev = u;
    u = prox_box(u - gamma_u * (WD.' * y1 + Phit(y2)), lower, upper);

    q_prev = q;
    q = prox_l2(q - gamma_q * (- y1), gamma_q, 1 - alpha);

    y1 = prox_l1_dual(y1 + gamma_y1 * (WD * (2 * u - u_prev) - (2 * q - q_prev)));

    y2 = prox_ball_l2_dual(y2 + gamma_y2 * (Phi(2 * u - u_prev)));
