function result = solve_gtv(shared_config, specific_config)
    ...
    problem_config.L = @(z) {shared_config.Phi(z{1}), ...
                             shared_config.G.weights .* shared_config.G.Diff * z{1}};
    problem_config.Lt = @(z) {shared_config.Phit(z{1} + shared_config.G.Diff.' * (shared_config.G.weights .* z{2}))};
    ...
    algorithm_config.Gamma_x = {1 / sqrt(shared_config.G.lmax * max(shared_config.G.weights))};
