...
optimal_grid_result_collection_tmp = cell(numel(shared_random_seed), 1);
...
for i = 1:numel(shared_random_seed)
    ...
    grid_param_skeleton.corrupted_graph.forward_weight_corruption.idx_to_corrupt.corruption_ratio = 0.3;
    ...
    optimal_grid_result_collection_tmp{i} = get_optimal_grid_result_collection(grid_param_skeleton, "parallel", []);

end

% Get the method names
field_names = fieldnames(optimal_grid_result_collection_tmp{1});

for i = 1:numel(field_names)

    % Create a struct to aggregate the optimal grid results over the shared random seeds for each method
    optimal_grid_result_collection.(field_names{i}) = cellfun(@(optimal_grid_result) optimal_grid_result.(field_names{i}).result, optimal_grid_result_collection_tmp, 'UniformOutput', false);

    % Concatenate the results over the shared random seeds along a new dimension at the tail
    optimal_grid_result_collection.(field_names{i}) = cat(ndims(optimal_grid_result_collection.(field_names{i}){1}) + 1, optimal_grid_result_collection.(field_names{i}){:});
...
for i = 1:numel(field_names)
    ...
    nmse_collection{i} = compute_nmse_from_result(optimal_grid_result_collection.(field_names{i}));

    % Average the NMSE over the random seed dimension
    averaged_nmse_collection{i} = mean(nmse_collection{i}, ndims(optimal_grid_result_collection.(field_names{i})));
