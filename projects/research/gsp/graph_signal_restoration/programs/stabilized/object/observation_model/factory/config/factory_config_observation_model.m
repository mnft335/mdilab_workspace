function config_observation_model = factory_config_observation_model(param, arg)
    ...
    switch param.type
        ...
        case "inpainting"
            ...
            config_observation_model.generate_observation_model = @(graph) generate_observation_model_inpainting(generate_signal_mask(graph.N), generate_signal_noise, param.std_dev);
        ...
        case "inpainting_without_noise"
            ...
            % Create a function handle that generates the observation model of an inpainting problem without noise
            config_observation_model.generate_observation_model = @(graph) generate_observation_model_inpainting(generate_signal_mask(graph.N), @(vector) zeros(size(vector)), 0);
